...
版本：1.4.0 · 交付日期：2026-09-06 · 竖屏 H5 / 标准浏览器 · 无真实广告（广告位为占位逻辑）、无登录、无 CDN、无运行时第三方依赖。
...
保卫战共 6 个线性解锁的关卡（12—24 波不等，6 的倍数波与末波有大王），难度逐关提升：命中预算收紧、阵型变刁钻、危险线上移、大王更频繁。每波清干净就能轻装翻篇；没清完会扣心（连续失手越扣越多：1、2、3……护盾挡不住），而且没打完的捣蛋鬼会保留残血、混进下一波一起压上来。最后一波没有下家，必须清完才算守住。残敌每轮照常前进一格，夜宵潮后改为每轮两格。普通敌人越线消耗护盾或生命，并逃回街尾重新排队——放走的怪还会回来；大王越线直接结束。

每次斩杀大王（第 6 / 12 / 18 波）获得一次改装三选一，此时可看广告让本次多选一项；瞄准阶段右下角也可主动看广告换一次三选一。广告每局最多 3 次，**当前为占位逻辑，不加载真实广告**。全局有一次换牌机会，也能放弃改装换一颗心。试试雷链＋冰糖、分裂＋穿透、爆破＋横扫、借墙＋回弹。

## 内容与进度

12 种机制型改装、4 种组合提示、3 位可解锁店长（桃桃「开门红」、芋圆「静电场」各有天然天赋）、5 类敌人、6 关保卫战（通关解锁下一关）、UTC+8 每日同题、无尽夜班、本机战绩、图鉴和纪念章。结算获得印章（高关卡有倍率），累计 8 / 20 印章解锁桃桃／芋圆，不要求广告或付费。
